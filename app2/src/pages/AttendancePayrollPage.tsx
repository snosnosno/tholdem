...
const AttendancePayrollPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingLog) {
      await updateWorkLog(editingLog.id, { ...form, totalMinutes });
    } else {
      await addWorkLog({ ...form, totalMinutes });
    }
    setModalOpen(false);
    setEditingLog(null);
    setForm({ date: '', staffId: '', clockIn: '', clockOut: '', wage: 10000, approved: false });
  };

  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <h1 className="text-3xl font-bold mb-6 text-gray-800">근태/페이롤 관리</h1>
      <button onClick={() => { setModalOpen(true); setEditingLog(null); setForm({ date: '', staffId: '', clockIn: '', clockOut: '', wage: 10000, approved: false }); }} className="btn btn-primary mb-4">+ 근무 기록 추가</button>
      {loading ? (
        <div>로딩 중...</div>
      ) : error ? (

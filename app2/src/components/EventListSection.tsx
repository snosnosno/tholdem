...
const EventListSection: React.FC = () => {
  ...
  const getApplicationCount = (eventId: string) => {
    return applications.filter((app: StaffApplication) => app.eventId === eventId).length;
  }
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4 overflow-x-auto">
      <div className="flex flex-col md:flex-row md:items-center gap-2 mb-4">
        <h2 className="text-xl font-bold flex-1">모집 공고 리스트 (isAdmin: {isAdmin ? 'true' : 'false'})</h2>
        <input
          type="text"
            ...
            {filteredEvents.map((ev) => (
              <tr key={ev.id} className="hover:bg-blue-50 transition">
                ...
                <td className="px-4 py-2 flex gap-2 flex-wrap">
                  {isAdmin && <button onClick={() => openEditModal(ev)} className="btn btn-secondary btn-xs">수정</button>}
                  {isAdmin && <button onClick={() => handleDelete(ev.id)} className="btn btn-danger btn-xs">삭제</button>}
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingEvent ? '공고 수정' : '공고 생성'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <textarea name="description" value={form.description || ''} onChange={handleChange} className="input-field w-full" rows={3}></textarea>
          </div>
